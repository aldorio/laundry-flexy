...
if(isset($_GET['delete'])){
  $id_user = $_GET['delete'];
  $queryDelete = mysqli_query($config, "DELETE FROM level WHERE id = $id_user");
  if($queryDelete) {
...
$queryEdit = mysqli_query($config, "SELECT * FROM level WHERE id = '$id_user'");
...
//   if(!isset($_GET['edit'])){
//     $insert = mysqli_query($config, "INSERT INTO level (name) VALUES('$name')");
//     header("location:?page=roles&tambah=berhasil");
//   }else{
//     $update = mysqli_query($config, "UPDATE level SET name='$name' WHERE id='$id_user'");
